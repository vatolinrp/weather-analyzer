...
public class DarkSkyFCGetterSpout extends BaseRichSpout {
  ...
  private static final String URL = "https://" + DARK_SKY_HOST + "/forecast/" + DARK_SKY_API_KEY + "/"+MINSK_LAT+","
      + MINSK_LON +",%s?exclude=daily,currently,flags";
  ...
  public void nextTuple() {
    try {
      ...
      for(HourlyDatum hourlyDatum : weatherElement.getHourly().getData() ) {
        if( unixTime <= hourlyDatum.getTime() ) {
      ...
      weatherConditionTO.setTemperature( oneHourForecast.getTemperature() );
      weatherConditionTO.setLocationKey( MINSK_CANNONICAL_LOCATION_KEY );
      Date date = new Date( oneHourForecast.getTime() * 1000L );
